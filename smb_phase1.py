...
def test(col_tuple, input_tuple, id_value, tablename):
    ...
    query = ''' select {} from "SMB"."{}" where id={} '''.format(
        columnstr, tablename, id_value)
...
def move_direct(tablename, id_value, sequence_id):
    # sequence_id logic
    sequence_initial = db.query(
        ''' select sequence_id from "SMB"."{}" where id={} and active=1 '''.format(tablename, id_value))[0][0]
    vacant_check = 0
    try:
        vacant_check = db.query('''select active from "SMB"."{}" where sequence_id={} and active=1 '''.format(
            tablename, sequence_id))[0][0]
    except:
        pass
    ...
    if(vacant_check == 1):
        ...
        if(sequence_final < sequence_initial):
            id_array = db.query(''' select id from "SMB"."{}" where sequence_id >={} and sequence_id<{} and active=1 order by sequence_id '''.format(
                tablename, sequence_final, sequence_initial))
            id_array = list(sum(id_array, ()))
            for i in id_array:
                qr1 = ''' UPDATE "SMB"."{}" SET sequence_id=sequence_id+1 where id={}  and active=1 '''.format(
                    tablename, i)
                db.insert(qr1)
        else:
            id_array = db.query(''' select id from "SMB"."{}" where sequence_id >{} and sequence_id<={} and active=1 order by sequence_id '''.format(
                tablename, sequence_initial, sequence_final))
            id_array = list(sum(id_array, ()))
            for j in id_array:
                qr2 = ''' UPDATE "SMB"."{}" SET sequence_id=sequence_id-1 where id={} and active=1 '''.format(
                    tablename, j)
                db.insert(qr2)
    query2 = ''' UPDATE "SMB"."{}" SET sequence_id={} where id={}  '''.format(
        tablename, sequence_id, id_value)
    db.insert(query2)
...
def upsert(col_tuple=None, input_tuple=None, flag='update', tablename=None, id_value=None):
    ...
    if(flag == 'update'):
        ...
        if(test_case['tag'] == True):
            ...
            check = db.query(''' select exists(select 1  from "SMB"."SMB_Aproval" where id={} and table_name='{}') '''.format(
                id_value, tablename))[0][0]
            if check and check != 'failed':
                querystr = ''' UPDATE "SMB"."SMB_Aproval" SET ({})= {} where id={} and table_name='{}' '''.format(
                    columnstr, input_tuple, id_value, tablename)
            else:
                querystr = '''INSERT INTO "SMB"."SMB_Aproval" ({}) VALUES{}; '''.format(
                    columnstr, input_tuple)
            status = db.insert(querystr)
            print(querystr)
            tableid = db.query(''' select max(tableid) from "SMB"."SMB_Aproval" where id={} and table_name='{}' '''.format(
                id_value, tablename))[0][0]
            time_stamp_update = db.insert(
                ''' update "SMB"."SMB_Aproval"  set updated_on='{}' where tableid={} '''.format(date_time, tableid))
    ...
    if(flag == 'add'):
        ...
        print(querystr)
        tableid = db.query(
            ''' select max(tableid) from "SMB"."SMB_Aproval" where table_name='{}' and flag='add' '''.format(tablename))[0][0]
        time_stamp_update = db.insert(
            ''' update "SMB"."SMB_Aproval"  set updated_on='{}' where tableid={} '''.format(date_time, tableid))

    if status != 'failed' and status != 'move_direct':
        status = 'success'
...
def move_records(tablename, col_tuple, value_tuple, flag, id_value=None, sequence_id=None):
    ...
    if(flag == 'update'):
        ...
        vacant_check = 0
        try:
            vacant_check = db.query('''select active from "SMB"."{}" where sequence_id={} '''.format(
                tablename, sequence_id))[0][0]
        except:
            pass
        ...
        query2 = ''' UPDATE "SMB"."{}" SET ({})= {} where id={}  '''.format(
            tablename, columnstr, value_tuple, id_value)
        print(query1)
        print(query2)
        db.insert(query1)
        ...
        if(status != 'failed'):
            db.insert(
                ''' delete from "SMB"."SMB_Aproval" where id={} '''.format(id_value))
            status = 'success'
        return status
    ...
    if(flag == 'add'):
        ...
        if(status != 'failed'):
            db.insert(
                ''' delete from "SMB"."SMB_Aproval" where tableid={} '''.format(id_value))
            status = 'success'
        return status
...
def get_old_json(tablename, id_data, flag):
    ...
    old_data_df.rename(columns={"Market_-_Country": "Market_Country",
                       "Market_-_Customer_Group": "Market_Customer_Group", "Zip_Code_(Dest)": "Zip_Code_Dest"}, inplace=True)
    ...
    try:
        ...
        if(df['flag'][0] != 'add'):
            for i in range(len(df)):
                a = [key for key, val in dict(
                    df.loc[i, lis] == old_data_df.loc[i, lis]).items() if val == False]
    ...
    if(flag == 'delete'):
        ...
        if(df.empty != True):
            df.rename(columns={"Market_-_Country": "Market_Country", "Market_-_Customer_Group":
                      "Market_Customer_Group", "Zip_Code_(Dest)": "Zip_Code_Dest"}, inplace=True)
            ...
            if(df['flag'][0] != 'add'):
                ...
                if(None not in id_data and len(id_data) > 1):
                    old_json, old_data_df = get_old_json(
                        tablename, tuple(id_data), flag)
...
def aprove_records():
    ...
    if(df['flag'][0] == 'delete'):
        ...
        query = '''UPDATE "SMB"."{}" SET "active"=0 WHERE "id" in {} '''.format(
            tablename, id_value)
        print(query)
...
"SMB"."{}" where "id" in {} '''.format(tablename, col_string, col_string, tablename, id_value)
        ...
        for i in range(0, len(df)):
            ...
            if(flag == 'update'):
                df2 = df.drop(
                    ['tableid', 'flag', 'status', 'table_name'], axis=1)
                col_tuple = tuple(list(df2.columns))
                value_tuple = tuple(df2.loc[i])

                status = move_records(
                    tablename, col_tuple, value_tuple, flag, id_value, sequence_id)
                ...
                try:
                    ...
                    sequence_id = db.query(
                        ''' select max(sequence_id) from "SMB"."{}" where "active"=1 '''.format(tablename))[0][0]+1
                except:
                    sequence_id = 1
                ...
                value_tuple = tuple(df2.loc[i])
                status = move_records(
                    tablename, col_tuple, value_tuple, flag, tableid)
...
def reject_records():
    ...
    id_list.append(0)
    id_tuple = tuple(id_list)
    query = ''' delete from "SMB"."SMB_Aproval" where tableid in {} '''.format(
        id_tuple)
    db.insert(query)
    return {"status": "success"}, 200
...
def get_record():
    ...
    query = '''select * from "SMB"."SMB - Base Price - Category Addition" where "id"={} '''.format(
        id_value)
...
def update_record1():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, BusinessCode,  Market_Country,
                   Product_Division, Product_Level_02, Document_Item_Currency, Amount, Currency)
    ...
    if(status['status'] == 'success'):
        email_status = email([status['tableid']], tablename, username=username)

    if(email_status == 'success' or status['status'] == 'move_direct'):
        return {"status": "success"}, 200
    else:
        return {"status": "failure"}, 500
...
def delete_record_baseprice():
    ...
    id_value, username = json.loads(
        request.data)['id'], request.headers['username']
...
def add_record1():
    ...
    input_tuple = (tablename, flag, username, BusinessCode,  Market_Country,
                   Product_Division, Product_Level_02, Document_Item_Currency, Amount, Currency)
...
def SMB_upload():
    ...
    df = smb_df[['id', 'BusinessCode', 'Market - Country', 'Product Division',
...
def fun_ignore_sequence_id(df, tablename):
    ...
    for ids in id_tuple:
        try:
            sequence_id.append(db.query(
                ''' select sequence_id from "SMB"."{}" where id={} '''.format(tablename, ids))[0][0])
        except:
            pass
...
def SMB_validate():
    ...
    col_tuple = ("table_name", "id", "sequence_id",
                 ...
                 "Amount",
                 "Currency")
    col_list = ['table_name', 'id', 'sequence_id', 'Username', 'BusinessCode', 'Market_Country',
                'Product_Division', 'Product_Level_02', 'Document_Item_Currency', 'Amount', 'Currency']
    ...
    for i in range(0, len(df)):
        status = upsert(col_tuple, tuple(
            df.loc[i]), flag, tablename, df['id'][i])
...
def get_record_minibar():
    ...
    query = '''SELECT * FROM "SMB"."SMB - Base Price - Category Addition - MiniBar" where id={}'''.format(
        id_value)
...
def update_record_category_minibar():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, BusinessCode, Customer_Group,
...
def add_record_mini():
    ...
    input_tuple = (tablename, flag, username, BusinessCode, Customer_Group, Market_Customer,
                   Market_Country, Beam_Category, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "flag",  "Username",
...
def SMB_upload_baseprice_minibar():
    ...
    df = smb_df[['id', 'BusinessCode', 'Customer Group', 'Market - Customer', 'Market - Country',
                 'Beam Category', 'Document Item Currency', 'Amount', 'Currency', 'sequence_id']]
    df['id'] = df['id'].astype(int)
    ...
    df3.columns = df3.columns.str.replace(' ', '_')
    df3.rename(columns={"Market_-_Country": "Market_Country",
               "Market_-_Customer": "Market_Customer"}, inplace=True)
...
def SMB_validate_baseprice_minibar():
    ...
    col_tuple = ("table_name", "id", "sequence_id", "Username",
                 ...
                 "Amount",
                 "Currency")
    col_list = ['table_name', 'id', 'sequence_id', "Username", 'BusinessCode', 'Customer_Group',
                'Market_Customer', 'Market_Country', 'Beam_Category', 'Document_Item_Currency', 'Amount', 'Currency']
...
def get_record_incoterm():
    ...
    query = '''SELECT * FROM "SMB"."SMB - Base Price - Incoterm Exceptions" where id={}'''.format(
        id_value)
...
def update_record_incoterm():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, Market_Country, Customer_Group, Incoterm1,
                   Product_Division, Beam_Category, Delivering_Mill, Document_Item_Currency, Amount, Currency)
...
def add_record_incoterm():
    ...
    input_tuple = (tablename, flag, username, Market_Country, Product_Division, Incoterm1,
                   Customer_Group, Beam_Category, Delivering_Mill, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "flag",  "Username",
...
def SMB_validate_baseprice_incoterm():
    ...
    col_tuple = ("table_name", "id", "sequence_id", "Username",
                 ...
                 "Document Item Currency",
                 "Amount", "Currency")
    col_list = ['table_name', 'id', 'sequence_id', 'Username', 'Market_Country', 'Product_Division', 'Incoterm1',
                'Customer_Group', 'Beam_Category', 'Delivering_Mill', 'Document_Item_Currency', 'Amount', 'Currency']
...
def get_record_extra_certificate():
    ...
    query = '''SELECT * FROM "SMB"."SMB - Extra - Certificate" where id={}'''.format(
        id_value)
...
def add_record_extra_certificate():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, BusinessCode, Certificate,
                   Grade_Category, Market_Country, Delivering_Mill, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "id", "sequence_id", "Username",
                 "BusinessCode",
                 "Certificate",
                 "Grade Category",
...
def smb_add_certificate():
    ...
    input_tuple = (tablename, flag, username, BusinessCode, Certificate, Grade_Category,
                   Market_Country, Delivering_Mill, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "flag", "Username",
                 "BusinessCode",
                 "Certificate",
                 "Grade Category",
...
def Upload_extra_certificate():
    ...
    try:
       ...
       "Document Item Currency", "Amount", "Currency" from "SMB"."SMB - Extra - Certificate" where "active"='1' order by sequence_id ''', con=con)
        ...
        df3.columns = df3.columns.str.replace(' ', '_')
        df3.rename(
            columns={"Market_-_Country": "Market_Country"}, inplace=True)
...
def validate_extra_certificate():
    ...
    col_tuple = ("table_name", "id", "sequence_id",
                 ...
                 "Amount",
                 "Currency")
    col_list = ['table_name', 'id', 'sequence_id', 'Username', 'BusinessCode', 'Certificate',
                'Grade_Category', 'Market_Country', 'Delivering_Mill', 'Document_Item_Currency', 'Amount', 'Currency']
...
def get_record_extra_certificate_minibar():
    ...
    query = '''SELECT * FROM "SMB"."SMB - Extra - Certificate - MiniBar" where id={}'''.format(
        id_value)
...
def update_record_extra_certificate_minibar():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, BusinessCode, Certificate, Market_Country,
                   Grade_Category, Customer_Group, Market_Customer, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "id", "sequence_id", "Username",
...
def add_record_extra_certificate_minibar():
    ...
    input_tuple = (tablename, flag, username, BusinessCode, Certificate, Market_Country,
                   Grade_Category, Customer_Group, Market_Customer, Document_Item_Currency, Amount, Currency)
...
def validate_extra_certificate_minibar():
    ...
    col_list = ['table_name', 'id', 'sequence_id', 'Username', 'BusinessCode', 'Certificate', 'Market_Country',
                'Market_Customer', 'Grade_Category', 'Customer_Group', 'Document_Item_Currency', 'Amount', 'Currency']
...
def get_record_delivery_mill():
    ...
    query = '''SELECT * FROM "SMB"."SMB - Extra - Delivery Mill" where id={}'''.format(
        id_value)
...
def update_record_delivery_mill():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, BusinessCode, Market_Country,
                   Delivering_Mill, Product_Division, Beam_Category, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "id", "sequence_id", "Username",
                 "BusinessCode",
                 "Market - Country",
                 "Delivering Mill",
...
def add_record_delivery_mill():
    ...
    input_tuple = (tablename, flag, username, BusinessCode, Market_Country, Delivering_Mill,
                   Product_Division, Beam_Category, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "flag", "Username",
                 "BusinessCode",
                 "Market - Country",
                 "Delivering Mill",
...
def get_record_delivery_mill_minibar():
    ...
    query = '''SELECT * FROM "SMB"."SMB - Extra - Delivery Mill - MiniBar" where id={}'''.format(
        id_value)
...
def update_record_delivery_mill_minibar():
    ...
    input_tuple = (tablename, id_value, sequence_id, username, Market_Country, Market_Customer_Group,
                   Market_Customer, Delivering_Mill, Product_Division, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "id", "sequence_id", "Username",
                 "Market - Country",
...
def add_record_delivery_mill_minibar():
    ...
    input_tuple = (tablename, flag, username, Market_Country, Market_Customer_Group, Market_Customer,
                   Delivering_Mill, Product_Division, Document_Item_Currency, Amount, Currency)
    col_tuple = ("table_name", "flag", "Username",
                 "Market - Country",
                 "Market - Customer Group", "Market - Customer",  "Delivering Mill",
                 "Product Division", "Document Item Currency", "Amount", "Currency")
...
def upload_delivery_mill_minibar():
    ...
    try:
       ...
       "Product Division", "Document Item Currency", "Amount", "Currency" from "SMB"."SMB - Extra - Delivery Mill - MiniBar" where "active"='1' order by sequence_id ''', con=con)
        ...
        df3.columns = df3.columns.str.replace(' ', '_')
        df3.rename(columns={"Market_-_Country": "Market_Country", "Market_-_Customer_Group": "Market_Customer_Group",
                   "Market_-_Customer": "Market_Customer", "Zip_Code_(Dest)": "Zip_Code_Dest"}, inplace=True)
...
def validate_delivery_mill_minibar():
    ...
    col_tuple = ("table_name", "id", "sequence_id",
                 ...
                 "Market - Customer Group", "Market - Customer", "Delivering Mill",
                 "Product Division", "Document Item Currency", "Amount", "Currency")
    col_list = ['table_name', 'id', 'sequence_id', 'Username', 'Market_Country', 'Market_Customer_Group',
                'Market_Customer', 'Delivering_Mill', 'Product_Division', 'Document_Item_Currency', 'Amount', 'Currency']
